...
# email shell script path
script_path = "/home/<netID>/answer_key/wrds/email_me.sh"
...
def send_kill_switch_email(script_path):
    subprocess.Popen(
        f"source {script_path} 'script triggered kill switch!'",
...
def main():
    ...
    for company in companies:
        for day in days:
            ...
            # artificially make an empty dataframe
            df = pd.DataFrame()

            # Check if the df produced by the sql query is empty
            if df.empty:
                print("Empty dataframe. Skipping to next company and day.")
                counter += 1
                if counter == 3:
                    print("Reached 3 empty files. Exiting and sending email notification.")
                    send_kill_switch_email(script_path)
                    sys.exit(1)
                else:
                    continue
            else:
